...
LINE    DB 80 DUP('-'),'$'  
;CHATMESS DB "YOU SENT CHAT INVITATION $"                   ;CHAT INVIT. MESSAGE WHERE NAME IS OTHER PLAYER NAME SEND
;CHATMESSR DB 'RECEIVED CHAT INVITATION TO START PRESS F1 $'      ;CHAT INVIT. MESSAGE WHERE NAME IS OTHER PLAYER NAME RECIEVE
;GAMEINVS DB "YOU SENT GAME INVITATION $"                   ;GAME INVIT. MESSAGE  SEND
;GAMEINVR DB "RECEIVED GAME INVITATION TO START PRESS F2 $"   ;GAME INVIT. MESSAGE  RECIEVE
CHATMESSR DB 30,26,'SENT YOU A CHAT INVITATION'
GAMEINVR DB 30,26,'SENT YOU A GAME INVITATION'
ERROR_MSG  DB 'CONNECTION IS NOT COMPLETED ','$'
...
NAMESS_G  DB 15,?,15 DUP('$')                                 ;GENERAL NAME 
P_POINTS_G DB 0
...
MESS_INTI_REG DB 'CHOOSE REG','$'
MESS_INTT_NOT DB 'NRI:1 RI:2','$'
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 ;/-----------------------------------------------------/SETUP PROC /-------------------------------------------
   ...
   ;-------------------------------------------------RECEIVE AT FIRST PROC--------------------------------------------------------------------------------

    ;------------------------------------------------------------------------ELSHABAB SHA8LOHAAAAA---------------------------------------------------------------
    YALA PROC FAR
    POP AX
    MOV CODE_ADD,AX
    POP AX 
   ...
   MOV VAR_CHOOSE,1
    ...
    B3D:
    SERIAL_SEND_STRING_50:
        mov dx , 3FDH		; Line Status Register
        AGAIN12:  	
            In al , dx 			;Read Line Status
      		test al , 00100000b
      	JZ AGAIN12                               ;Not empty
    ...
    ;If empty put the VALUE in Transmit data register
  		...
  		out dx , al
      DEC CL
    JNZ SERIAL_SEND_STRING_50
    
        mov dx , 3FDH		; Line Status Register
        AGAIN1211:  	
            In al , dx 			;Read Line Status
      		test al , 00100000b
      	JZ AGAIN1211                               ;Not empty
...
Receiving proc far
    ...
    inc charposX2
    MOVE_CURSOR charposY2, charposX2  
    cmp charposX2 , 70 
    jnz herex
...
REC_INLINE proc far
    ...
    mov dx , 03f8h
    in al , dx
    mov RecChar_INLINE, al
    
    ;;;;;;;
    cmp al , 13d                           ; if enter is clicked get new line
    jnz here2x11                    
    cmp charposY2_IN , 23                    ;checkIF YOU WANT TO CLEAR THE WINDOW
    jnz contx11
  ;   CLR_LOW_INLINE
  ;  CLR_UP_INLINE
    mov charposX2_IN , 15
    ...
    ;btb3ha 3la el shasha
    inc charposX2_IN
    MOVE_CURSOR charposY2_IN, charposX2_IN  
    cmp charposX2_IN , 70 
    jnz herex11
    ;if 0 increment y and return x to 0
    mov charposX2_IN , 0
   ...
   ;; print that char
    mov  ah ,9 
    mov bh,0
    mov al , RecChar_INLINE
    mov cx,1
    mov bl,73H
    int 10h
    Chk11: ;; lw mfesh data y5rog
    ret
REC_INLINE endp

CLR_UP_INLINE macro                     ;CLEAR THE UPPER PART OF THE MSG
  push bx
  mov ax , 0600h
  mov bh , 07
  mov cl , 15D                     ; FROM COL N 
  mov ch , 20d                     ; FROM ROW N  
  mov dl, 40d                      ; TO COL N
  mov dh, 21d                      ; TO ROW N
  int 10h
  pop bx
...
CLR_LOW_INLINE macro                     ;CLEAR THE LOWER PART OF THE MSG
  push bx
  mov ax , 0600h
  mov bh , 07
  mov cl , 15D                     ; FROM COL N 
  mov ch , 21d                     ; FROM ROW N
  mov dl, 40d                      ; TO COL N
  mov dh, 23d                      ; TO ROW N
  int 10h
  pop bx
...
MAIN PROC FAR  
    ...
    CALL PRINT_L
    READ_USERNAME NAMESS_G         ;read username
    CALL PRINT_L
    DISPLAY_MESS INMESS2           ;display mess2
    CALL PRINT_L
    READ_POINTS PMESS,P_POINTS_G ;read points
    CALL PRINT_L
  ...
  ;  CALL CLEAR_SCREEN_PROC                   ;CLEAR SCREEN
    ...
    ; MAIN SCREEN FOR PLAYER 1
     ...
     CALL PRINT_L
;------------------------------------------------------------------------------DETERMINE WHICH MODE AM I---------------------------------------------------------
      CALL YALA 


  ;///////////////----------------------------------------------------PUT GENERAL INTO SPECIFIC(NAMES-POINTS)----------------------------------------------------
  ;---------------------NAMES AND POINTS
  CMP DORY,1
  JNZ HOT_ESM_P2
  ...
  ;GENERAL_INST NAMEMESS+2,NAMESS_G+2,NAMESS_G+1 
  MOV AH,NAMESS_G+1
  MOV NAMEMESS+1,AH
  MOV AH,P_POINTS_G
  MOV P1_POINTS,AH
  ...
  INC DI
  LOOP TAN_6
  ;GENERAL_INST NAMEMESS2+2,NAMESS_G+2,NAMESS_G+1
  MOV AH,NAMESS_G+1
  MOV NAMEMESS2+1,AH
  MOV AH,P_POINTS_G
  MOV P2_POINTS,AH
...
;-----------------------------------------------------------------CHATTING MODE----------------------------------------------------------------------------------------
   ...
   mov cx , 0
    ...
    POPF
    POP AX
    jz loop111 ; lw mfesh haga edast recieve
...
;-----------------------------------------------------------------GAME MODE-------------------------------------------------------------------------------------------
   ...
   ;----------------------------------------------------------------------SENDING NAME AND INTIALIZE POINTS-----------------------------------------------------------------

   ;PLAYER 1 SEND/////////// PLAYER 2 RECEIVE
    CMP DORY,1
    JNZ ANA_P2
   ; GENERAL_INST SEND_VALUE+2,NAMEMESS+2,NAMEMESS+1
   ...
   LOOP TAN_16
    ...
    ANA_P2:
    CALL RECEIVO
   ; GENERAL_INST NAMEMESS+2,REC_VALUE,REC_A_SIZE
   mov CL,REC_A_SIZE
   mov ch,0
   lea SI,REC_VALUE
   LEA DI, NAMEMESS+2
   ...
   LOOP TAN_8
    ...
    ;PLAYER 2 SEND/////////// PLAYER 1 RECEIVE
    CMP DORY,1
    JNZ ANA_P2_1
    CALL RECEIVO
    ;GENERAL_INST NAMEMESS2+2,REC_VALUE,REC_A_SIZE
    mov CL,REC_A_SIZE
    mov ch,0
     lea SI,REC_VALUE
    LEA DI,NAMEMESS2+2
    ...
    ANA_P2_1:
    ; GENERAL_INST SEND_VALUE+2,NAMEMESS2+2,NAMEMESS2+1
    mov CL,NAMEMESS2+1
    mov ch,0
    lea SI,NAMEMESS2+2
    LEA DI,SEND_VALUE+2
    ...
    CMP DORY,1
    JNZ ANA_P2_PO
    ;GENERAL_INST SEND_VALUE+2,PMESS+2,TWO
    mov CL,2
    mov ch,0
    lea SI,PMESS+2
    LEA DI,SEND_VALUE+2
    ...
    ANA_P2_PO: 
    CALL RECEIVO
    ;GENERAL_INST PMESS+2,REC_VALUE,TWO
    mov CL,2
    mov ch,0
    lea SI,REC_VALUE
    LEA DI,PMESS+2
    ...
    RET_NUM_COUNT PMESS+2
    MOV P1_POINTS,Dl
    RET_NUM_COUNT PMESS+3
    mov al,10d
    mov bl,P1_POINTS
    mul bl
    add P1_POINTS,Dl
    ...
    CALL RECEIVO
    ;GENERAL_INST PMESS2+2,REC_VALUE,TWO
    mov CL,2
   mov ch,0
    lea SI,REC_VALUE
  LEA DI,PMESS2+2
  ...
  LOOP TAN_14
    ...
    ANA_P2_PO1: 
    ;GENERAL_INST SEND_VALUE+2,PMESS2+2,TWO
    mov CL,2
   mov ch,0
   lea SI,PMESS+2
  LEA DI,SEND_VALUE+2
...
;------------------------------------------------------------END COMMAND--------------------------------------------------------------------------------
          ...
          ;------PLAYER 2--------------
           ...
           ;----------------------POWER UP 5--------------------
            ...
            MOMO019:
              ...
              MOMO0199:                               ;MOVE NEW VAL INTO TARG VAL
            MOV TARG_CHAGE_P1,1
            SUB P1_POINTS,8D
            JMP CONTT
  ...
  ;----------------------------------------MEEN KESEB-------------------------------------------
   ...
   ; DrawLine 
    ...
    POPF
    POP AX
    jz MY_loop ; lw mfesh haga edast recieve
    
    ;;lw ktb haga t3ala nb3tha
    mov SendChar_INLINE, al
    cmp al , 13d ; if enter is clicked get new line
    jnz here211
    ;;check 34an lw 3auz ams7 el shasha
    cmp charposY_IN , 21
    ...
    MOVE_CURSOR charposY_IN, charposX_IN 
